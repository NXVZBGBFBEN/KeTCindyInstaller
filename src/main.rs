...
async fn main() -> Result<()> {
    let project_directories = directories::ProjectDirs::from("", "NXVZBGBFBEN", "KeTCindyInstaller")
    ...
    match argument.subcommand {
        Subcommand::Install { nodeps: _, packages } => {
            for package in packages {
                let download_location = project_directories.cache_dir().join("packages").join(package.to_string());
